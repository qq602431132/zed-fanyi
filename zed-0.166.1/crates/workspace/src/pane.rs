...
impl Pane {
                        ...
                        PopoverMenu::new("pane-tab-bar-split")
                            ...
                            .menu(move |cx| {
                                ContextMenu::build(cx, |menu, _| {
                                    menu.action("向右拆分", SplitRight.boxed_clone())
                                        .action("向左拆分", SplitLeft.boxed_clone())
                                        .action("向上拆分", SplitUp.boxed_clone())
                                        .action("向下拆分", SplitDown.boxed_clone())
                                })
                                .into()
                            }),
                    )
    ...
    fn render_tab_bar(&mut self, cx: &mut ViewContext<'_, Pane>) -> impl IntoElement {
        ...
        let navigate_backward = IconButton::new("navigate_backward", IconName::ArrowLeft)
            ...
            .tooltip({
                let focus_handle = focus_handle.clone();
                move |cx| Tooltip::for_action_in("返回", &GoBack, &focus_handle, cx)
            });
        ...
        let navigate_forward = IconButton::new("navigate_forward", IconName::ArrowRight)
            ...
            .tooltip({
                let focus_handle = focus_handle.clone();
                move |cx| Tooltip::for_action_in("前进", &GoForward, &focus_handle, cx)
            });
