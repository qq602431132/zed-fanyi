                ...
                menu.context(self.focus_handle.clone()).map(|menu| {
                    if is_read_only {
                        menu.when(is_dir, |menu| {
                            menu.action("查找内部", Box::new(NewSearchInDirectory))
                        })
                    } else {
                        menu.action("新建文件", Box::new(NewFile))
                            ...
                            .when(is_local && cfg!(target_os = "macos"), |menu| {
                                menu.action("文件管理器打开", Box::new(RevealInFileManager))
                            })
                            .when(is_local && cfg!(not(target_os = "macos")), |menu| {
                                menu.action("文件管理器打开", Box::new(RevealInFileManager))
                            })
                            .when(is_local, |menu| {
                                menu.action("默认程序打开", Box::new(OpenWithSystem))
                            })
                            .action("终端打开", Box::new(OpenInTerminal))
                            .when(is_dir, |menu| {
                                menu.separator()
                            ...
                            .when(!is_root & !is_remote, |menu| {
                                menu.action("删除到回收站", Box::new(Trash { skip_prompt: false }))
                            })
                            .when(!is_root, |menu| {
                                menu.action("彻底删除", Box::new(Delete { skip_prompt: false }))
                            })
