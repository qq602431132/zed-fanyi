...
impl ExtensionsPage {
    pub fn new(workspace: &Workspace, cx: &mut ViewContext<Workspace>) -> View<Self> {
        cx.new_view(|cx: &mut ViewContext<Self>| {
            ...
            let query_editor = cx.new_view(|cx| {
                ...
                input.set_placeholder_text("查找扩展...", cx);
                input
            });
                ...
                h_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .child(
                                Button::new(SharedString::from(extension.id.clone()), "卸载")
                                    .on_click({
                                        let extension_id = extension.id.clone();
                                        move |_, cx| {
                                            ExtensionStore::global(cx).update(cx, |store, cx| {
    ...
    fn buttons_for_entry(
        ...
        if has_dev_extension {
            ...
            return (
                Button::new(SharedString::from(extension.id.clone()), "安装"),
                None,
            );
        }
        ...
        match status.clone() {
            ExtensionStatus::NotInstalled => (
                Button::new(SharedString::from(extension.id.clone()), "安装").on_click(
                    cx.listener({
                        let extension_id = extension.id.clone();
                        move |this, _, cx| {
                            this.telemetry
            ...
            ExtensionStatus::Installing => (
                Button::new(SharedString::from(extension.id.clone()), "安装").disabled(true),
                None,
            ),
            ExtensionStatus::Upgrading => (
                Button::new(SharedString::from(extension.id.clone()), "卸载").disabled(true),
                Some(
                    Button::new(SharedString::from(extension.id.clone()), "Upgrade").disabled(true),
                ),
            ),
            ExtensionStatus::Installed(installed_version) => (
                Button::new(SharedString::from(extension.id.clone()), "卸载").on_click(
                    cx.listener({
                        let extension_id = extension.id.clone();
                        move |this, _, cx| {
                            this.telemetry
                ...
                v_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .gap_2()
                            .justify_between()
                            .child(Headline::new("扩展").size(HeadlineSize::XLarge))
                            .child(
