                ...
                .trigger(
                    IconButton::new("toggle_editor_selections_icon", IconName::CursorIBeam)
                        ...
                        .when(!self.toggle_selections_handle.is_deployed(), |this| {
                            this.tooltip(|cx| Tooltip::text("选择选项", cx))
                        }),
                )
                ...
                .menu(move |cx| {
                    ...
                    let menu = ContextMenu::build(cx, move |menu, _| {
                        menu.context(focus.clone())
                            ...
                            .action("展开", Box::new(SelectLargerSyntaxNode))
                            .action("折叠", Box::new(SelectSmallerSyntaxNode))
                            .action("在上方添加光标", Box::new(AddSelectionAbove))
                            .action("在下方添加光标", Box::new(AddSelectionBelow))
                            .separator()
                            .action("转到字符", Box::new(ToggleOutline))
                            .action("转到行/列", Box::new(ToggleGoToLine))
                            .separator()
                            .action("下一个问题", Box::new(GoToDiagnostic))
                            .action("上一个问题", Box::new(GoToPrevDiagnostic))
                            .separator()
                            .action("下一个代码块", Box::new(GoToHunk))
                            .action("上一个代码块", Box::new(GoToPrevHunk))
                            .separator()
                            .action("向上移动行", Box::new(MoveLineUp))
                            .action("向下移动行", Box::new(MoveLineDown))
                            .action("复制所选内容", Box::new(DuplicateLineDown))
                    });
                    Some(menu)
                })
        });
                ...
                .trigger(
                    IconButton::new("toggle_editor_settings_icon", IconName::Sliders)
                        ...
                        .when(!self.toggle_settings_handle.is_deployed(), |this| {
                            this.tooltip(|cx| Tooltip::text("编辑器选项", cx))
                        }),
                )
                .anchor(AnchorCorner::TopRight)
